...
void bmx_task(void* params)
{
    ...
    while (1)
    {
        ...
        ESP_LOGI(TAG_BME280, "Read Values: temp = %f, pres = %f, hum = %f", temp, pres, hum);
        _bme280_res res = { temp, pres, hum };
        send_data(&res);
...
void app_main(void) {
    init();
    init_udp_and_lamp();
    xTaskCreate(&bmx_task, "bmxtask", 4048, NULL, 6, NULL);
}
